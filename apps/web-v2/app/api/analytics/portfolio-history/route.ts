...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // wallets (which only ever have the live point).
    ...
    // live `getPortfolio()` read from the canonical SSOT. The cron
    // snapshot is the historical record (we keep every prior day from
    // it), but the rightmost point on the chart is always rebuilt from
    // `getPortfolio()` so it matches `balance_check`. If the live read
    // fails we fall back to the cron snapshot when we have one,
    // otherwise we drop the trailing point silently.
...
function computeChange(
  ...
  const last = snapshots.at(-1)?.netWorthUsd ?? 0;
