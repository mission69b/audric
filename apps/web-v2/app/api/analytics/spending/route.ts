...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const e of payEvents) {
      ...
      if (purchaseServiceIds.has(service)) {
        continue;
      }
      ...
      const amount =
        typeof details.amount === "number"
          ? details.amount
          : typeof details.amountUsdc === "number"
            ? details.amountUsdc
            : 0;
      if (amount <= 0) {
        continue;
...
function categorizeService(serviceId: string): string {
  ...
  if (id.startsWith("http")) {
    try {
      id = new URL(id).pathname.toLowerCase();
    } catch {
      // use raw
    }
  }
  ...
  if (id.includes("lob") || id.includes("postcard") || id.includes("letter")) {
    return "Mail";
  }
  if (
    id.includes("brave") ||
  ...
  if (id.includes("weather") || id.includes("openweather")) {
    return "Utilities";
  }
  if (id.includes("heygen") || id.includes("video")) {
    return "Video";
  }
  if (
    id.includes("openai") ||
  ...
  if (id.includes("deepl") || id.includes("translate")) {
    return "Translation";
  }
  return "Other";
}
