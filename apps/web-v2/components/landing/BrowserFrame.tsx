...
export function BrowserFrame({ url, children, className }: BrowserFrameProps) {
  return (
    <div
      className={[
        "rounded-md overflow-hidden border border-border bg-card shadow-[0_1px_0_rgba(0,0,0,0.02),0_20px_40px_-20px_rgba(0,0,0,0.10)]",
        className ?? "",
      ]
        .filter(Boolean)
        .join(" ")}
    >
      <div className="flex items-center gap-1.5 px-3.5 py-2.5 border-b border-border bg-muted">
        <span
          aria-hidden="true"
        ...
        <span
          ...
          className="w-2 h-2 rounded-full bg-muted-foreground"
        />
        <span className="flex-1 text-center font-mono text-[10px] tracking-[0.1em] text-muted-foreground">
          {url}
        </span>
      </div>
      {children}
