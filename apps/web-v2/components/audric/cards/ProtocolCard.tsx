...
// ProtocolCard — `protocol_deep_dive` tool renderer. Ported from
// `apps/web/components/engine/cards/ProtocolCard.tsx` by Phase 5a.4
...
function parseScore(raw: string | number | undefined): number | null {
  ...
  return Number.isNaN(n) ? null : Math.min(Math.max(n, 0), 10);
}
...
export function ProtocolCard({ data }: { data: ProtocolData }) {
  ...
  return (
    <CardShell
      ...
      badge={
        data.category ? (
          <span className="text-[9px] font-mono uppercase text-fg-muted">
            {data.category}
          </span>
        ) : undefined
      }
    >
      ...
      <div className="space-y-1 font-mono text-[11px]">
        ...
        {data.tvlChange1d != null && (
          <DetailRow label="24h Change">
            <TrendIndicator value={data.tvlChange1d} />
          </DetailRow>
        )}
        {data.tvlChange7d != null && (
          <DetailRow label="7d Change">
            <TrendIndicator value={data.tvlChange7d} />
          </DetailRow>
        )}
        ...
        {data.revenue24h != null && (
          <DetailRow label="Revenue (24h)">
            ${fmtUsd(data.revenue24h)}
          </DetailRow>
        )}
        ...
        {data.chains && data.chains.length > 0 && (
          <DetailRow label="Chains">
            {data.chains.slice(0, 5).join(', ')}
            {data.chains.length > 5 ? ` +${data.chains.length - 5}` : ''}
          </DetailRow>
        )}
      </div>
      ...
      {data.riskFactors && data.riskFactors.length > 0 && (
        <div className="mt-2 pt-2 border-t border-border-subtle/50 space-y-0.5">
          {data.riskFactors.map((risk, i) => (
            <div
              key={i}
              className="text-[10px] text-fg-muted flex items-start gap-1"
            >
