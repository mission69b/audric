...
// StakingCard — `volo_stats` tool renderer. Ported from
// `apps/web/components/engine/cards/StakingCard.tsx` by Phase 5a.4
    ...
    <CardShell title="Volo Staking">
      ...
      <div className="space-y-1 font-mono text-[11px]">
        <DetailRow label="Exchange Rate">
          1 vSUI = {fmtAmt(data.exchangeRate, 4)} SUI
        </DetailRow>
        <DetailRow label="Total Staked">
          {fmtLargeNumber(data.totalStaked)} SUI
        </DetailRow>
        <DetailRow label="Total vSUI">
          {fmtLargeNumber(data.totalVSui)}
        </DetailRow>
      </div>
    </CardShell>
  );
