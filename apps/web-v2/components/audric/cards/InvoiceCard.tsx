...
// InvoiceCard — `create_invoice` + `list_invoices` tool renderer. Ported
// from `apps/web/components/engine/cards/InvoiceCard.tsx` by Phase 5a.4
// (renderer migration sweep, 2026-05-19). Verbatim except QrCode import
// path (now under `cards/shared/` instead of `dashboard/`).
...
function StatusPill({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    pending: {
      label: 'Pending',
      cls: 'bg-warning-bg text-warning-fg border border-warning-border',
    },
    overdue: {
      label: 'Overdue',
