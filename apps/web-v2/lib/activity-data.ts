...
function normalizeAction(type: string): string | null {
  ...
  if (t.includes("save") || t.includes("deposit")) {
    return "save";
  }
  ...
  if (t.includes("pay") || t.includes("service")) {
    return "pay";
  }
  if (t.includes("claim")) {
    return "claim";
  ...
  if (t.includes("follow_up") || t.includes("briefing") || t.includes("goal")) {
    return null;
  }
  return null;
}
...
async function fetchAppEventBuckets(
  address: string,
  since: Date
...
async function fetchChainBuckets(
  address: string,
  since: Date
): Promise<Map<string, DayBucket>> {
  ...
  async function paginateQuery(
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      try {
        ...
        if (reachedOldTxns || !res.hasNextPage || !res.nextCursor) {
          break;
        }
...
function mergeBuckets(
  a: Map<string, DayBucket>,
  b: Map<string, DayBucket>
): DayBucket[] {
...
export async function fetchActivitySummary(
  ...
  const actionMap = new Map<
    string,
    { count: number; totalAmountUsd: number }
  >();
  ...
  for (const e of events) {
    ...
    const amount =
      typeof details.amountUsd === "number"
        ? details.amountUsd
  ...
  try {
    ...
    if (user) {
      ...
      yieldEarnedUsd = yieldSnaps.reduce(
        (s, snap) => s + (snap.yieldEarnedUsd ?? 0),
        0
      );
    }
  } catch {
