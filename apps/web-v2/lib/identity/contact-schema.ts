...
/**
 * web-v2 copy of `apps/web/lib/identity/contact-schema.ts`.
 * Intentional duplicate during v0.7c Phase 6 — apps/web is SSOT
 * until v0.7e deletion sweep. Edit both copies together.
 *
 * --- ORIGINAL HEADER ---
 *
...
function liftLegacyContact(
  legacy: z.infer<typeof LegacyContactSchema>
): Contact {
  return {
    name: legacy.name,
...
export function parseContact(raw: unknown): Contact | null {
  ...
  if (legacy.success) {
    return liftLegacyContact(legacy.data);
  }
