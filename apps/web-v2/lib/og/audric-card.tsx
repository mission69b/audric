...
/**
 * Shared Open Graph / Twitter card renderer for audric.ai.
 *
 * Follows the t2000 house pattern (see `t2000/apps/web` +
 * `apps/gateway` opengraph-image.tsx and the canonical design at
 * `t2000-AFI/audric/og-audric.svg`): #0A0A0A canvas, 16px inset
 * hairline, AudricMark + "audric" wordmark top-left, a teal status
 * pill, a two-line Geist headline, a Geist subtitle, and a Geist Mono
 * footer with a divider. Fonts come from Google Fonts at render time
 * via `loadGoogleFont` (same helper t2000/gateway use), so nothing is
 * vendored. One renderer powers root / pay / share so the brand stays
 * identical and a single edit updates every card.
 */
...
interface AudricCardOptions {
  ...
  line2: string;
  /** Teal status pill text, e.g. "LIVE · SUI". */
  pill: string;
  /** Supporting line under the headline. */
  subtitle: string;
}

export async function renderAudricCard({
  pill,
  line1,
  line2,
  subtitle,
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        <div
          style={{ width: 8, height: 8, borderRadius: 4, background: ACCENT }}
        />
        <div
          style={{
            display: "flex",
