...
/**
 * Like `assertOwns`. Used to additionally accept the case where the caller
 * had `targetAddress` in their `WatchAddress` watch-list (saved contact /
 * watched wallet), but the WatchAddress feature was retired in S.254
 * (2026-05-22). The function is preserved as a thin alias so the 3 call
 * sites (portfolio route, auth-fetch, internal-auth) don't need to
 * change shape — semantically equivalent to `assertOwns` now.
 *
 * SPEC 30 Phase 1A.5 origin — closes the unauthenticated-read class.
 *
 * Returns `null` on success. Returns 400 when the address is malformed,
 * 403 when the caller doesn't own the target.
 */
export function assertOwnsOrWatched(
  verified: VerifiedJwt,
  targetAddress: string
): Promise<NextResponse | null> {
  return Promise.resolve(assertOwns(verified, targetAddress));
}
