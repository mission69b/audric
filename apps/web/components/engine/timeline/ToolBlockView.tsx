...
export function ToolBlockView({
  ...
  //
  // The block.source is set by `useEngine.upsertToolBlock` calls from
  // `dashboard-content.tsx:handleRegenerateToolCall` to 'user' when the
  // user clicked a Regenerate button (vs 'llm' for LLM-driven dispatches
  // and 'pwr' for engine post-write refreshes). Surfacing this distinction
  // visually is important for the fastpath: there's no LLM narration after
  // a regen ("Done — here's your new image" doesn't fire because the
  // fastpath bypasses the LLM round-trip), so without this marker the
  // user can't tell whether the new card is a fresh regen or part of the
  // original turn. One-line subtle pill, no buttons or other affordances.
  const isUserRegen = block.source === 'user';
  ...
  return (
    <div className="space-y-1">
      ...
      {isUserRegen && isSettled && !isStreaming && (
        <div className="pl-7 -mb-1">
          <span
            className="inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-[0.10em] text-fg-muted px-2 py-0.5 rounded-sm border border-border-subtle bg-surface-subtle"
            aria-label="This result was regenerated by you"
          >
            ↻ Regenerated
          </span>
        </div>
