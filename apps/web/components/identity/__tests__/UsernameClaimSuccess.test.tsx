 ...
 *
 * [S.118 follow-up 2026-05-08] All display + clipboard + share-card
 * assertions updated from `.audric.sui` → `@audric` to match the
 * SuiNS V2 short-form alias the component now renders. The on-chain
 * NFT name (the `fullHandle` returned by the API mint route) is still
 * `<label>.audric.sui` — only the user-facing render flips. Both forms
 * resolve to the same address via SuiNS RPC.
 *
 * Coverage:
 *   1. Renders headline + full display handle + tagline.
 *   2. Copy button writes the FULL DISPLAY handle (`alice@audric`) to clipboard.
...
describe('UsernameClaimSuccess', () => {
  ...
  describe('copy button', () => {
    it('writes the FULL DISPLAY handle to clipboard (not bare label, not 0x)', () => {
      render(<UsernameClaimSuccess label="alice" walletAddress="0xabc" />);
      fireEvent.click(screen.getByTestId('username-claim-success-copy'));
      expect(clipboardWrites).toEqual(['alice@audric']);
    });
    ...
    it('updates aria-label between idle and copied state', () => {
      render(<UsernameClaimSuccess label="alice" />);
      const button = screen.getByTestId('username-claim-success-copy');
      expect(button.getAttribute('aria-label')).toBe('Copy alice@audric');
      fireEvent.click(button);
  ...
  describe('show QR toggle', () => {
    it('expands to reveal SuiPayQr + handle + truncated address', () => {
      ...
      expect(panel.textContent).toContain('alice@audric');
  ...
  describe('share to X', () => {
    it('href has spec-locked tweet template (handle + audric.ai/label + 🪪)', () => {
      ...
      expect(decoded).toBe(
        'I just claimed my Audric Passport — alice@audric 🪪\n\nPay me on Sui: https://audric.ai/alice',
      );
    });
    ...
    it('encodes special characters in the label safely', () => {
      ...
      const href = link.getAttribute('href') ?? '';
      const decoded = decodeURIComponent(href.replace('https://x.com/intent/tweet?text=', ''));
      expect(decoded).toContain('alice-smith@audric');
