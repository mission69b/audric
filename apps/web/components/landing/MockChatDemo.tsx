...
const DEMOS: Record<string, DemoMessage[]> = {
  ...
  'Send postcard': [
    ...
    { role: 'assistant', text: 'I\'ll create a birthday postcard for your mum. Generating design + personal message...<br/><br/>🎨 <em>Watercolour flowers with "Happy Birthday Mum"</em><br/><br/>Print &amp; mail: <strong>$1.05</strong>. Send it?', delay: 1800 },
    { role: 'user', text: 'Send it', delay: 3600 },
    { role: 'assistant', text: 'Done! Postcard on its way. Paid <strong>$1.05</strong> from your USDC balance. Delivery in 3–5 days. 💌', delay: 4800 },
  ],
  'Payment link': [
...
export function MockChatDemo() {
  ...
  return (
    <div className="w-full max-w-md border border-border-subtle rounded-xl overflow-hidden bg-surface-page shadow-[0_8px_40px_rgba(0,0,0,0.08)]">
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border-subtle">
        <span className="w-1.5 h-1.5 rounded-full bg-success-solid" />
        <span className="text-xs font-medium text-fg-primary">Audric</span>
        <span className="ml-auto font-mono text-[9px] text-fg-muted">Online</span>
      </div>
        ...
        {messages.slice(0, visible).map((msg, i) => (
          <div
            ...
            className={`max-w-[85%] px-3.5 py-2.5 text-[12px] leading-[1.5] animate-[fadeSlideIn_0.3s_ease-out_both] ${
              msg.role === 'user'
                ? 'self-end bg-fg-primary text-fg-inverse rounded-2xl rounded-br-sm'
                : 'self-start bg-surface-card text-fg-primary rounded-2xl rounded-bl-sm'
            }`}
      ...
      {/* Chips */}
      <div className="px-4 py-2 border-t border-border-subtle flex gap-1.5 overflow-x-auto scrollbar-none">
        {CHIPS.map((chip) => (
          <button
            key={chip}
            ...
            className={`shrink-0 font-mono text-[10px] px-2.5 py-1 border rounded-full transition cursor-pointer ${
              chip === activeDemo
                ...
                : 'border-border-subtle text-fg-secondary hover:text-fg-primary hover:border-fg-primary'
            }`}
          >
            {chip}
          </button>
      ...
      {/* Sign-in badge */}
      <div className="px-4 py-1.5 text-center bg-surface-card border-t border-border-subtle">
        <span className="font-mono text-[9px] text-fg-muted">Sign in with Google to start</span>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-t border-border-subtle">
        <button
          onClick={login}
          className="flex-1 text-left bg-surface-card border border-border-subtle rounded-lg px-3 py-2 text-[12px] text-fg-muted cursor-pointer hover:border-fg-primary transition"
        >
          Ask anything about your money...
        </button>
        <button
          onClick={login}
          className="shrink-0 w-8 h-8 rounded-lg bg-fg-primary text-fg-inverse flex items-center justify-center cursor-pointer hover:opacity-80 transition"
        >
