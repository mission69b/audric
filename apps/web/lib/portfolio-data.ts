...
export async function fetchWalletBalances(address: string): Promise<WalletBalances> {
  try {
    ...
    const allocations: Record<string, number> = { USDC: usdc, SUI: sui, ...assets };
    ...
    console.error('[portfolio-data] fetchWalletBalances error:', err instanceof Error ? err.message : err);
    return { ...EMPTY_WALLET };
  }
}
...
export async function fetchPositions(address: string): Promise<PositionSummary> {
  try {
    ...
    console.error('[portfolio-data] fetchPositions error:', err instanceof Error ? err.message : err);
