...
export function deriveSmartCards(state: AccountState): SmartCardData[] {
  ...
  const saveableUsdc = state.usdc ?? state.cash;
  if (saveableUsdc > 5) {
    const rate = state.bestRate ?? (state.savingsRate > 0.005 ? state.savingsRate : 0);
    if (rate > 0) {
      const monthlyEarnings = (saveableUsdc * rate) / 12;
      cards.push({
        type: 'idle-funds',
        icon: '💰',
        title: `$${Math.floor(saveableUsdc)} idle — could earn $${monthlyEarnings.toFixed(2)}/mo at ${(rate * 100).toFixed(1)}%`,
        body: '',
  ...
  if (cards.length === 0) {
    if (state.cash === 0 && state.savings === 0) {
      ...
      if (earnRate > 0.005 && state.savings > 0) parts.push(`Earning ${(earnRate * 100).toFixed(1)}% on $${Math.floor(state.savings)}`);
      cards.push({
        type: 'all-good',
        icon: '✅',
