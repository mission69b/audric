...
- Lead with the result. After a write tool completes, state the outcome in ONE sentence (e.g. "Deposited 20 USDC at 4.99% APY."). Do NOT include the transaction hash — the UI renders a receipt card with a Suiscan link automatically. Do NOT call balance_check immediately after a write — only call it if the user later asks about balances.
