...
//
// Both event shapes ship as native members of `@t2000/engine`'s `SSEEvent`
// union as of v1.19.0 (P9.6 release). This file re-exports them as
// type aliases so existing audric imports (`import { FormSchema } from
// '@/lib/engine/sse-types'`) keep working — every alias resolves to the
// engine's canonical types. New code should import directly from
// `@t2000/engine`.
// ───────────────────────────────────────────────────────────────────────────

import type { SSEEvent, FormField as EngineFormField, FormSchema as EngineFormSchema } from '@t2000/engine';

type ExtractByType<U, T extends string> = U extends { type: T } ? U : never;

export type ProactiveTextSseEvent = ExtractByType<SSEEvent, 'proactive_text'>;
export type ProactiveType = ProactiveTextSseEvent['proactiveType'];

export type FormFieldKind = EngineFormField['kind'];
export type FormField = EngineFormField;
export type FormSchema = EngineFormSchema;
export type PendingInputSseEvent = ExtractByType<SSEEvent, 'pending_input'>;
