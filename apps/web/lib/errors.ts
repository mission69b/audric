...
const MOVE_ABORT_MAP: Record<number, { message: string; chips?: { label: string; flow: string }[] }> = {
  ...
  46000: { message: 'Not enough wallet balance to repay. Withdraw a small amount from savings first, then repay.', chips: [{ label: 'Withdraw $1', flow: 'Withdraw $1' }, { label: 'Check balance', flow: 'balance' }] },
};
