...
> **ARCHIVED 2026-05-11 (SPEC 23A-P0).** This runbook is now historical. The interactive harness ("v2") shipped to 100% on 2026-04-30, was monitored clean for 11 days, and the legacy renderer (`LegacyReasoningRender`) plus the rollout-percent infra (`rolloutPercent`, `bucketFor`, `NEXT_PUBLIC_INTERACTIVE_HARNESS_ROLLOUT_PERCENT`) were all deleted in the SPEC 23A-P0 legacy harness rip. The Upstash audit confirmed 0 sessions still pinned to legacy at rip time. Keep this file as a reference template for future graduated-rollout playbooks (the FNV-1a bucket pattern + dial-back recipe is reusable), but do NOT follow the dial-flip steps below — those env vars no longer exist.
>
