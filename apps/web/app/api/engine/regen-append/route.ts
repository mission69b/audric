...
import type { Message, ContentBlock, SessionData } from '@t2000/engine';
...
interface AppendRegenSuccess {
  messages: Message[];
}

interface AppendRegenError {
  error: string;
  status: number;
}
...
export function appendRegenToMessages(
  ...
  // Defense: collision check on newToolUseId
  ...
  // The very next message MUST be a user message containing the
  // matching tool_result. If the immediately-next message is missing
  // or wrong shape, the original call never completed — refuse the
  // regen because we can't safely anchor.
