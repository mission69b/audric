// Copy of web-v3 `lib/ai/prompts.ts`. Keep in sync when web-v3 changes.
// All tool/memory/artifact segments are here (not just the ones the current
// route uses) so `systemPrompt()` stays identical to web-v3 as features land.
// web-v3's `@vercel/functions` Geo and `ArtifactKind` imports don't exist on
// native, so they're redeclared below. The artifact/title generation prompts
// (codePrompt/sheetPrompt/titlePrompt/updateDocumentPrompt) aren't part of the
// system prompt and are omitted.
