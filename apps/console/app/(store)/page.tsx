...
export default async function HomePage() {
  ...
  return (
    <>
      {/* ── Hero — centered over the radial glow (the 3-way panel was
          deleted 2026-07-19: /join owns onboarding; one prompt is the only
          artifact a visitor needs from the hero). ── */}
      <section className="relative pt-12 text-center">
        <div
          aria-hidden="true"
          className="-top-24 pointer-events-none absolute left-1/2 h-[460px] w-[720px] max-w-full -translate-x-1/2"
          style={{
            background:
              "radial-gradient(46% 46% at 50% 40%, rgba(0,114,245,0.14) 0%, transparent 70%)",
            filter: "blur(20px)",
          }}
        />
        <div className="relative">
          ...
          <div className="mt-7 flex flex-wrap justify-center gap-2.5">
            ...
            <Link className="ag-btn ag-btn--ghost" href="/join">
              Join — hire or sell
            </Link>
          </div>
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {topSellers.map((s) => {
                  ...
                  // Sellers WITHOUT an Agent ID have no profile page —
                  // linking them 404s. Their receipts still rank; the name
                  // renders as plain text until they claim.
                  const hasProfile =
                    Boolean(row) || serviceNames.has(s.seller.toLowerCase());
                  return (
                    <tr
                    ...
                    >
                      <td className="px-5 py-3 max-md:px-4">
                        {hasProfile ? (
                          ...
                          <span className="font-medium font-mono text-[13px] text-fg-muted">
                            {name}
                          </span>
                        )}
                      </td>
