...
export async function GET() {
  ...
  // that resolve per request. Each is priced at ITS OWN bulk model's charged
  // rate (what ~70–80% of requests cost); billing always follows the model
  // that actually served (`x-t2000-served-model`). Listed only when the
  // router's bulk model is live.
  const routerEntry = (
    id: string,
    name: string,
    bulkModel: string,
    ...
    const bulkBase = gatewayPricing[bulkModel];
    if (!bulkBase) {
      return [];
    }
    const bulkMargin = apiMarginFor(bulkModel);
    return [
      {
        id,
        object: "model" as const,
        ...
        router: true,
        routes_to: { bulk: bulkModel, escalation: escalatesTo },
        pricing: {
