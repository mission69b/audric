 ...
 *
 * Audric sends one asset (USDC), so there's no currency-substitution gate — the
 * "which asset?" ambiguity simply doesn't exist. Pure + deterministic by design
 * (the human tap is the other gate). Reused by the Recipes `payService` path.
 */
...
function intentKey(input: { to: string; amount: number }): string {
  return `${input.amount}:${input.to.toLowerCase()}`;
}
...
/**
 * Run the full pre-dispatch screen for a USDC send. Call in onAllow before
 ...
 */
export function screenSend(input: {
  to: string;
  amount: number;
}): ScreenResult {
  const pf = preflightSend({ ...input, asset: "USDC" });
  if (!pf.valid) {
  ...
  if (isDuplicateSend(input)) {
    return {
      ok: false,
      reason: `You just sent ${input.amount} USDC to this address moments ago — I won't repeat it automatically. Ask again if you really want to send it twice.`,
    };
  }
