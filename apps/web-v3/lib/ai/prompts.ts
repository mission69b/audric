...
export const stockPrompt = `Stocks / equities: for a US-listed STOCK or ETF (price, market cap, P/E, EPS, 52-week range, dividend yield, beta, analyst buy/hold/sell ratings), call \`stock_analysis\` with the company name or ticker — NOT web_search (this is exact + live). Call once per ticker, then synthesize the numbers in your own words (a markdown table for an overview or a multi-stock comparison); cite "Finnhub". It covers US equities only — for non-US tickers, or if it returns no match, fall back to web_search. (This is stocks; for crypto use the crypto tools above.)`;
