...
test.describe("Suggested Actions", () => {
  test("suggested actions prefill the composer (injection-only)", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    if (count > 0) {
      const label = ((await suggestions.first().textContent()) ?? "").trim();
      await suggestions.first().click();

      // Prefill-only (CHIP_REVIEW_3): the chip injects into the composer and
      // does NOT auto-send / navigate.
      await expect(page.getByTestId("multimodal-input")).toHaveValue(label);
    }
  });
});
