...
> Last refreshed 2026-05-23 ~21:45 AEST post-S.283 (PIPELINE-AUDIT-PHASE-2 S5 closed as no-op — hypothesis disproven on read-through; the 9 BV protocols were deliberately pruned DOWN from 26 for burst-safety per `protocols.ts` header, not aspirationally added). **PIPELINE-AUDIT-PHASE-2 is now COMPLETE.** Earlier today: S.282 (S3 canvases on shared SWR); S.281 (S2 rename); S.280 (S1 split, patch v2.19.2); S.279.1 (S.279 self-audit cleanup, patch v2.19.1); S.279 (CLI-CONTACTS-CLEANUP, minor v2.19.0); S.278 (SPEC 272 Lever 1 cron user-batching); S.277 ("Earns Its Keep" audit + cuts, engine 2.18.1); S.273 (payment-link URL prefix fix); S.269 (template-divergence + V07E_INVOICE_DEPRECATION); S.267 (engine 2.15.0). Originally rewritten 2026-05-22 ~17:30 AEST post-S.255. Previous Phase-6.5-era detail is preserved in git history (`git log -- HANDOFF_NEXT_AGENT.md`); the document is now a tight current-state-of-the-world view, not a session-by-session log. Session-by-session detail lives in `t2000/audric-build-tracker.md`.

---

## 🎯 Where we are right now (1-paragraph)

**S.283 closed 2026-05-23 ~21:45 AEST — PIPELINE-AUDIT-PHASE-2 S5 closed as NO-OP (hypothesis disproven on read-through). Founder asked "do we really need telemetry that might be over engineered right" before scoping the §7.1 30-day per-protocol fire-rate telemetry the audit said was the S5 prerequisite. Pre-scoping read-through of `packages/engine/src/blockvision/defi/protocols.ts` surfaced the file's own header history (lines 7-17): the 9 protocols were **deliberately pruned DOWN from BV's 26-protocol superset** in v0.50.2 because v0.50.1's 26-call burst was triggering wallet-endpoint 429s — NOT aspirationally added. The audit's "5 of 9 are aspirational and never fire" hypothesis was wrong by construction. All 5 bespoke normalisers (`normalizeBluefin` / `normalizeHaedal` / `normalizeSuistake` / `normalizeWalrus` / `normalizeSuinsStaking`) cover real live Sui mainnet protocols with distinct response shapes the generic walker can't replace. Without telemetry we can't prove any specific normaliser is dead; with founder declining telemetry, the loop closes — nothing to ship, S5 is done as a NO-OP. **PIPELINE-AUDIT-PHASE-2 is COMPLETE** (S1+S2+S3 shipped; S5 closed as no-op; S4+S6+S7 were already deferred/rejected at audit time). One observation worth logging (NOT acted on): the 5 bespoke normalisers have zero direct test coverage — that's a regression-gap, separate slice if/when it bites. Lesson for future read-only audits: cross-check "drop dead code" hypotheses against the existing code's change-log comments before flagging as a viable track. Docs-only change (audit doc + tracker + HANDOFF + SSOT inventory); no engine bump. Natural next pickups: D8 V07E_STALE_FINCONTEXT_WRITE_REFUSAL Phase 2 / B1 marketing landing redesign / OPS-1 (founder action: delete dead `audric-web` Vercel project).**

**Pre-S.283 1-paragraph (preserved):** S.282 shipped 2026-05-23 ~21:30 AEST — PIPELINE-AUDIT-PHASE-2 S3 (canvases → shared `usePortfolio` SWR cache). The audit projected "~30 LoC per canvas × 8 canvases = 240 LoC, stagger over PRs." Ground-truth grep of every `*Canvas*.tsx` for `authFetch` + `/api/portfolio` showed only **2 canvases** actually fetch portfolio data — `FullPortfolioCanvas` (1 of 3 parallel fetches) and `WatchAddressCanvas` (sole fetch). The other 6 canvases own different concerns: heatmap fetches `/api/analytics/activity-heatmap`, spending fetches `/api/analytics/spending`, portfolio-timeline fetches `/api/analytics/portfolio-history`, and the 4 simulators (yield projector, health sim, receive address, DCA planner) take props. Scope corrected from 8 to 2 canvases; single atomic PR instead of staggered (the staggering rationale evaporates below the 240-LoC threshold). `WatchAddressCanvas` migration was a clean structural win: dropped 6 `useState` + 1 `useEffect` + 1 `authFetch` lifecycle, replaced with `usePortfolio(address)` + `useMemo` for the coins-list transform; grep confirms **0** `useState`/`useEffect`/`authFetch` references remain in the file. `FullPortfolioCanvas` was surgical: only the portfolio leg migrated; the heatmap + spending fetches keep their local `useEffect` (now with renamed `analyticsLoading` so portfolio data can render independently of analytics loading). Pre-existing type bug in `usePortfolio.PortfolioWalletCoin.balance` (declared `number`, wire shape is `string` — silent since empty-state didn't parse balance) fixed by aligning to the wire shape, with a comment for the historian. SSOT win: BalanceHero (empty-state) + FullPortfolio + WatchAddress now all share `portfolio:${address}` SWR cache key — opening FullPortfolio after BalanceHero already loaded is instant. Net delta **−28 LoC** (audit predicted +240). All gates green: typecheck ✅, biome ✅ on the hook file (canvases are biome-excluded per the ported-code rule), 17/17 tests pass. Audric-only commit; no engine bump. Next pickup: S5 is the last remaining PIPELINE-AUDIT-PHASE-2 track but it's GATED on §7.1 telemetry (need 30-day per-protocol fire-rate data to identify dead BV normalisers) — telemetry hooks aren't yet wired in `blockvision/defi/index.ts`. Likely natural pivots: D8 V07E_STALE_FINCONTEXT_WRITE_REFUSAL Phase 2 / B1 marketing landing redesign / OPS-1 (founder action: delete the dead `audric-web` Vercel project).**

**Pre-S.282 1-paragraph (preserved):** S.281 shipped 2026-05-23 ~21:05 AEST — PIPELINE-AUDIT-PHASE-2 S2. The audit flagged 3 portfolio-named files (`lib/portfolio.ts`, `lib/portfolio-data.ts`, `lib/profile-portfolio.ts`) as a cognitive trap — open the folder and there appear to be 3 parallel portfolio fetchers. Read-through proved the diagnosis was a naming problem, not an architecture problem: `portfolio.ts` (400 LoC) is the canonical orchestrator with 8 consumers; `portfolio-data.ts` (167 LoC) is a single-purpose NAVI lending fetcher used only by `portfolio.ts`; `profile-portfolio.ts` (93 LoC) is a thin adapter that calls canonical `getPortfolio()` and reshapes for the public `/[username]` page with silent-null degradation. Action: `git mv lib/portfolio-data.ts → lib/navi-positions.ts`. The new filename is self-evident ("this fetches NAVI lending positions"), removing the cognitive collision with `portfolio.ts`. 4 string references updated: 2 in `portfolio.ts` (import + type re-export), 1 doc-comment in `app/api/chat/route.ts`, plus header docblock rewritten. Audric live docs updated in lockstep: `CLAUDE.md` directory tree + Unified Data Layer table + Critical Rule #8 (which was doubly stale — wrong filename AND named `getCanonicalPortfolio` which is the deleted legacy apps/web fn name, the web-v2 fn is `getPortfolio`); `.cursor/rules/audric-canonical-portfolio.mdc` header + cross-ref; `.claude/rules/runbooks.md` portfolio-regression-matrix trigger. PURE REFACTOR: zero logic touched. Typecheck ✅, Biome ✅ on 3 changed files, all 17 web-v2 tests pass. No engine bump needed (this is audric-only). Pending PIPELINE-AUDIT-PHASE-2: S3 (canvases → `usePortfolio` SWR; ~240 LoC across 8 canvases, stagger over PRs), S5 (drop dead BV normalisers; gated on 30-day telemetry). Broader doc-rot in `audric/CLAUDE.md` (still shows `apps/web/` directory tree, references archived ESLint rules) was left for a separate cleanup slice per surgical-changes discipline.**

**Pre-S.281 1-paragraph (preserved):** S.280 shipped 2026-05-23 ~20:35 AEST — PIPELINE-AUDIT-PHASE-2 S1. The 2009-LoC `packages/engine/src/blockvision-prices.ts` monolith was the single biggest source of cognitive load in the engine — it did 7+ jobs (BV retry layer, circuit breaker, wallet portfolio, token prices, DeFi aggregator, per-protocol fetcher, generic walker, bespoke normalisers, cache-clear helpers) in one file. Split into 11 focused files under `packages/engine/src/blockvision/` (retry · wallet · prices · admin · defi/{index,protocols,walker,normalizers,one-protocol} + a re-export index). Largest single file is now `wallet.ts` at 504 LoC — 4× smaller than the monolith. Public API preserved by a 21-LoC re-export shim (`blockvision-prices.ts` → `blockvision/index.ts`), so all 17 internal importers + the engine package `index.ts` + 7 test files keep working with zero churn. PURE REFACTOR: no logic changes, every test passes byte-for-byte (1319/1319 engine · 595/595 SDK · 130/130 MCP · 35/35 CLI). Patch release v2.19.2 across SDK + engine + CLI + MCP. The audit doc at `t2000/spec/active/AUDIT_ON_CHAIN_PIPELINE_2026-05-23.md` now has a ship-log appendix marking S1 ✅ shipped, with S2 (`lib/portfolio*.ts` collapse), S3 (canvases → `usePortfolio` SWR), S5 (drop dead BV normalisers) still pending. Next pickup options: S2 / S3 / D8 V07E_STALE_FINCONTEXT_WRITE_REFUSAL Phase 2 / B1 marketing landing redesign.**

**Pre-S.280 1-paragraph (preserved):** S.279.1 shipped 2026-05-23 ~20:10 AEST — S.279 self-audit + residue cleanup. v2.19.0 left two pieces of work undone: (1) MCP `t2000_send` `dryRun: true` with a SuiNS recipient (e.g. `alex.sui`) silently rejected with `CONTACT_NOT_FOUND` because the dryRun path called `agent.contacts.resolve()` directly while the live execute path resolved SuiNS via the new `T2000.resolveRecipient`; (2) 6 doc/desc spots still steered users toward the deprecated contacts path with no SuiNS surfacing. Fix: promoted `T2000.resolveRecipient` from `private` → `public` so MCP's dryRun uses the same resolution code as `agent.send()`. Added SuiNS-in-dryRun regression tests + ContactManager.add() now also fires the once-per-process deprecation warning (was previously only on resolve(), so `t2000 contacts add Tom` was silent until later use). Updated MCP `t2000_send` + `t2000_contact_add` + `t2000_contact_remove` + `t2000_contacts` tool descriptions to lead with SuiNS / flag contacts as deprecated. Updated SDK README + CLI README + PRODUCT_FACTS.md. Patch release v2.19.1 (SDK + engine + CLI + MCP). Tests: 595/595 SDK · 1319/1319 engine · 35/35 CLI · 130/130 MCP green.**

**Pre-S.279.1 1-paragraph (preserved):** S.279 shipped 2026-05-23 ~19:25 AEST — CLI-CONTACTS-CLEANUP. `T2000.send()` now accepts SuiNS names (`alex.sui`, `team.alex.sui`) in addition to hex addresses and the legacy `contacts.json` alias map. Priority order: hex → `.sui` → contact alias. The engine's `sui/address.ts` SuiNS resolver was promoted to the SDK (`packages/sdk/src/utils/suins.ts`) — one canonical implementation, the engine's `sui/address.ts` is now a thin re-export shim so all 11 existing engine import paths keep working unchanged. `contacts.json` now prints a one-shot DEPRECATION warning to stderr the first time a contact alias resolves in a process (sunset target: next major SDK). New error code: `T2000Error('SUINS_NOT_REGISTERED')` for well-formed but unregistered SuiNS names. New `SendResult.suinsName?` field surfaces the resolved name on CLI receipts ("Sent $10 USDC → alex.sui (0x8b3e…d412)"). CLI send help text + CLI_UX_SPEC updated. End-to-end mainnet smoke: `funkii.sui` → `0x40cd…3e62` ✅.
...
| 7.5 | ~~**PIPELINE-AUDIT-PHASE-2**~~ | **✅ COMPLETE 2026-05-23** | done | — | Phase 1 audit at `t2000/spec/active/AUDIT_ON_CHAIN_PIPELINE_2026-05-23.md` shipped read-only; Phase 2 fully closed. **✅ S1 (S.280)** split 2009-LoC `packages/engine/src/blockvision-prices.ts` into 11 focused files (patch v2.19.2). **✅ S2 (S.281)** renamed `lib/portfolio-data.ts` → `lib/navi-positions.ts`. **✅ S3 (S.282)** wired the 2 portfolio-fetching canvases to shared `usePortfolio` SWR cache; net delta −28 LoC. **✅ S5 (S.283)** closed as NO-OP — read-through of `packages/engine/src/blockvision/defi/protocols.ts` header (lines 7-17) showed the 9 protocols were deliberately pruned DOWN from BV's 26-protocol superset in v0.50.2 for burst-safety, not aspirationally added; the audit's "5 are dead" hypothesis was wrong by construction. DEFER (still deferred): S4 (engine cache trio factor), S6 (Pyth-pricing replacement). REJECT: S7 (full BV → native migration). The whole Phase 2 needs production telemetry (BV outage frequency, per-protocol fire-rate, BV monthly cost) before locking the Phase 3 ship plan. **Sequencing locked 2026-05-23:** S.274 ships first (different lens — tool surface vs. data plumbing). Pairs naturally — S.272 fix (cron rate-limit hardening) overlaps the Phase 1 audit's S1+S5 tracks (they touch the same 2009-LoC file). Bundling decision: keep S.272 + S.274 + PIPELINE-AUDIT-PHASE-2 as 3 separate ships in series, NOT one omnibus. S.272 fix may overlap S1 split work — coordinate. |
...
2. **The next agent-ownable work, in order:**
   ...
   - ~~**(c) PIPELINE-AUDIT-PHASE-2**~~ ✅ COMPLETE — S1 ✅ (S.280) + S2 ✅ (S.281) + S3 ✅ (S.282) shipped 2026-05-23; S5 ✅ closed as no-op (S.283). See `t2000/spec/active/AUDIT_ON_CHAIN_PIPELINE_2026-05-23.md`. The whole track is done. (Coordinate any future `blockvision/` namespace work with S.272 Lever 2+3 IF they end up shipping.)
