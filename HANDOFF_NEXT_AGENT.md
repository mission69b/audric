...
| 🟠 RE-BASELINED + PARTIALLY SHIPPED S.253 (3 commits 2026-05-22 ~19:00–20:00 AEST) | H1 | **v0.7e Phase 2 → STRATEGIC RE-FRAMING per S.253 audit (2026-05-22 ~17:30–18:30 AEST). Then 3 commits SHIPPED tonight (~19:00–20:00 AEST) closing the cron + legal + litepaper gaps.**

**SHIPPED 2026-05-22 ~20:00 AEST — 3 commits, ~3,000 LoC net additions to web-v2:**
...
**Remaining work on the compressed trajectory (refined post-ship):**
1. **Marketing landing port (LARGEST remaining piece)** — port `apps/web/app/page.tsx` + 15 components in `apps/web/components/landing/` (~1,748 LoC) + `useZkLogin` adapter. Web-v2 uses `use-zklogin.ts` (different naming) and has its own AppProviders chain; this is NOT a wholesale `cp` — requires per-component adaptation. **Effort: ~2-4h focused work; recommended as standalone next-session ticket.** Alternative: ship a minimal placeholder page.tsx (~50 LoC, sign-in + links to /chat, /litepaper, /privacy, /terms) to make audric.ai flippable now, deferring the full marketing site to a later sprint.
2. **Chat-shell rewrites to apps/web/next.config.ts** — add 4 rewrites (`/new` → web-v2/chat, `/chat/:path*` → web-v2/chat/:path*, `/api/chat` → web-v2/api/chat, `/api/transactions/:path*` → web-v2/api/transactions/:path*). **Effort: ~30 min code + post-deploy smoke.** **PRODUCTION ROUTING IMPACT** — runbook says founder-owned. Smoke each path after deploy; rollback is per-rewrite (comment 1 line).
3. **Founder ops gates** — (a) copy `CRON_SECRET` from `audric-web` to `audric-web-v2` Vercel project env (until then web-v2's 5 new crons return 401 — apps/web's still run; no production gap, just dual-write inactive on web-v2 side). (b) DNS flip — point audric.ai apex at audric-web-v2 Vercel project. (c) `git rm -rf apps/web`.

**Revised end-to-end (post-tonight): ~1-2d remaining focused work + founder ops + 48h soak. Cron + legal + litepaper gaps closed.**
