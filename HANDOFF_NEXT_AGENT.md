...
|---|---|---|---|---|---|
| 1 | ~~**S.263-SMOKE**~~ | ~~Re-smoke compound write with @audric recipient~~ — **✅ VERIFIED** (founder smoke 2026-05-23 ~10:10 AEST). All 4 targets green: (a) `swap 5 USDC to SUI then send 1 SUI to funkiii@audric` → ONE PTB, ONE tap; (b) `send 1 USDC to funkii.sui` → worked; (c) `send 1 USDC to funkiii@audric` → worked; (d) `send 1 USDC to bogusssss@audric` → 400 at prepare layer with exact contract message. S.263 fully landed. | — | — |
