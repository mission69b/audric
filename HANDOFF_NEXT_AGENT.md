...
|---|---|---|---|---|---|
| 0 | **S.269** | **Template-divergence audit — IN FLIGHT** | ~1 day, read-only | None — audit ships independently | Audit doc at `t2000/spec/active/AUDIT_V07E_TEMPLATE_DIVERGENCE_2026-05-23.md`. 8 surfaces (engine ToolContext wiring · auth seam · `lib/audric/` 4802 LoC · chat surfaces 5822 LoC · tool-result-router + canvas template registry · system prompt parity · SWR / cache invalidation · direct `process.env` reads). Sharp constraints: 1-page exec at top, 3 buckets only (EARNS KEEP / DEBT / FOOT-GUN), 1 concrete recommendation. S.268, S.270, S.271 categorized inside the audit; founder picks scope from the recommendation. |
