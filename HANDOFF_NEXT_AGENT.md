...
| `t2000-web-env-gate` | **NEW** OPEN (P3, ~1-2h, t2000 repo) — add `lib/env.ts` with Zod schema for t2000/apps/web (mirror `audric/apps/web/lib/env.ts` v0.53.x ~375 lines) + boot-time validation via `instrumentation.ts` + ESLint `no-restricted-syntax` ban on raw `process.env`. T2000 marketing site is the last app in this monorepo without the env-gate; the C.1 stats route had to inline a `resolveSuiAddress` workaround because of this. Once the gate ships, that workaround can be deleted in favor of `env.T2000_OVERLAY_FEE_WALLET` (Zod enforces non-empty + valid-Sui-address at boot). | v0.7e- |
