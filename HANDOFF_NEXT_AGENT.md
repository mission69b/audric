...
| 🟠 RE-BASELINED S.253 (2x correction) | H1 | **v0.7e Phase 2 → STRATEGIC RE-FRAMING per S.253 audit (2026-05-22 ~17:30–18:30 AEST).** Two layers of correction this session: (1) First audit (against SPEC) claimed 3/5 Phase 6.5 P0s shipped + 2 partial. (2) Reading the TRACKER (which I should have done first) revealed **ALL of Phase 6.5 Groups A+B+C SHIPPED 2026-05-20 in S.199/S.200/S.201** with intentional design deviations (rate-limit stays in-memory by design; ChatGate inline via AuthenticatedChat wrapper). The SPEC's "NOT STARTED" header was never patched. See `spec/active/SPEC_V07C_PHASE_6_5_CHAT_PARITY.md` §0.1 for the corrected truth table. **Compounding correction:** v0.7c is essentially closed (S.215: *"7d soak collapsed into Group E smoke + production verification; web-v2 production has been native-HITL-only since the v0.7c Phase 6.5 chat-flip"*). v0.7d MemWal landed in Phase 6 Block A (S.221). v0.7e Phase 1A shipped (S.238). **Outstanding work that's REAL:** (1) Founder smoke check: navigate audric.ai/chat in a browser, confirm whether apps/web's `/new` or web-v2's `/chat` serves the response. `apps/web/next.config.ts` has rewrites for `/pay`, `/settings`, `/api/portfolio`, `/api/analytics`, `/[username]` but NOT for `/new`/`/chat`/`/api/engine`/`/api/transactions` — runbook v3 line 86 says these "flip in Session 6 as the founder-owned ops step" which can't be verified from code alone. (2) Based on smoke result: either ship the 4 chat-shell rewrites (~30 min if needed) OR confirm they're at a different config layer (Vercel dashboard). (3) Port marketing + `(legal)/*` + `litepaper` + 5 crons (especially `financial-context-snapshot` UFC) to web-v2 (~2-3d copy-port). (4) DNS flip — point audric.ai apex at audric-web-v2 Vercel project. (5) `git rm -rf apps/web`. **Revised end-to-end: ~3-5d focused work + 48h soak. Not 25-36d.** Prior H1 framing (Phase 2 = 6-8d single block of `git mv` + fn-injection) is fully SUPERSEDED. The S.251-S.252 audit work remains valid forward-planning IF the post-cutover-cleanup tier still scopes it. | **~3-5d end-to-end** (compressed from 25-36d locked) | Founder smoke check of audric.ai/chat routing — confirms whether chat-shell rewrites already shipped at non-code layer OR still need to ship |
