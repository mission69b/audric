...
| `apps-web-dead-rewritten-routes-cleanup` | **NEW** OPEN (P2, ~1-2h) — delete 4 additional dead-rewritten routes in `apps/web`: `/api/internal/payments`, `/api/portfolio`, `/api/analytics/portfolio-history`, `/api/analytics/activity-summary`. All four are rewritten unconditionally to `audric-web-v2.vercel.app` by `apps/web/next.config.ts:139-141,143` — local route handlers in apps/web are dead code at runtime. Same S.228 pattern: verify web-v2 counterpart exists + functionally equivalent → audit-first (zero test imports, rewrite is unconditional) → delete → typecheck/lint/test → commit → post-deploy smoke. Should land before v0.7e drafting so the v0.7e Tier C migration starts with the smallest possible apps/web surface. | Pre-v0.7e |
