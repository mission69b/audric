...
| 🔴 HIGH | H1 | **v0.7e Phase 2** — chat-shell cutover + 1A.2/1A.3 absorption + fn-injection refactor. **D-2/D-5/D-7 LOCKED 2026-05-22 ~06:30 AEST (S.244); D-2 RE-LOCKED 2026-05-22 ~07:50 AEST (S.245) → B+ DELETE entirely (engine tools deleted, Phase 5 RESTORED to v0.7e). PRE-EXECUTION AUDIT SHIPPED 2026-05-22 ~14:00 AEST (S.251) → `spec/active/V07E_PHASE_2_PRE_EXECUTION_AUDIT.md` surfaced 3 founder-blocking D-questions (NOT just R-1 as previously thought): Q1 Vitest L-8A vs L-8B; Q2 feature-loss matrix for 6 `/api/engine/*` routes; Q3 legacy `/api/history` migrate vs delete. Audit also recommends scope-split (2.A engine+lib+fn-injection ~3-4d + 2.B cutover+absorbed-deletes ~3-4d) over single block. Effort honesty: 5-7d (SPEC) / 6-9d (surface map) / 7-10d (audit) for single block.** | ~7-10d single block OR ~3-4d × 2 split | (1) Founder lock Q1 + Q2 + Q3 (~10 min review of the audit doc); (2) optional ratify D-3 voice / D-6 build-id default-DELETE; (3) optional stale-doc reconciliation |
