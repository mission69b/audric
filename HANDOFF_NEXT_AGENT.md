...
[`spec/active/BENEFITS_SPEC_v07c.md`](../t2000/spec/active/BENEFITS_SPEC_v07c.md) — v1.0 LOCKED 2026-05-18. Phase 0 closed; Phase 1 Day 1a/1b/1c/1d closed + post-Day-1d audit closed (G2 + G3 closed); **Phase 2 Day 2a closed** (P2.0a + P2.0b + P2.0c kickoff + minimal `/api/audric-chat` route shipped, end-to-end engine round-trip verified, all 3 standing gates green); **Day 2b READY**.
...
| Phase 2 — First read-tool round-trip + AI Gateway + intent-dispatcher spike + Agent + OTel | 🟢 **Day 2a CLOSED, Day 2b READY** | **Day 2a (P2.0a + P2.0b + P2.0c + chat route)**: ported full `verifyJwt` + Google JWKS + Enoki address derivation from `apps/web/lib/auth.ts` into `lib/audric-auth.ts`; added Zod-validated `lib/env.ts` with `instrumentation.ts` boot-time validation; added `@t2000/engine@2.7.3` + `@prisma/client@^7.5.0` + `@prisma/adapter-neon@^7.8.0` + `@neondatabase/serverless@^1.1.0`; `lib/prisma.ts` instantiates PrismaClient with Neon WS adapter, importing the generated client from `../web/lib/generated/prisma/client` (audric/web owns schema; web-v2 consumes); new `/api/audric-chat` POST handler at `app/(chat)/api/audric-chat/route.ts` translates `engine.submitMessage()` EngineEvent generator → AI SDK v6 UIMessageStream parts via `createUIMessageStream({execute})`; end-to-end smoke verified (JWKS verify 172ms / Enoki derive 267ms / Anthropic round-trip 1.5s); all 3 standing gates green. See S.168. |
