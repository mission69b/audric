...
|---|---|---|---|---|---|
| 1 | **S.262-SMOKE** | **Re-smoke compound writes on prod** — verify atomic Payment Intent | ~5 min founder | None | Post-S.262 Vercel deploy. Run `swap 1 SUI for USDC then save it into NAVI` on `audric.ai`. Expect: ONE tap-to-confirm, ONE PTB on Suiscan with both legs in the same digest. Bonus smoke: `swap 5 USDC to SUI then send 1 SUI to alice.audric.sui` (3-op DAG). If still 2 taps → escalate to engine-side soft guard (out-of-scope for now; S.263 candidate). |
...
| 3 | ~~**AUDRIC-BUMP-1**~~ | ~~Bump web-v2 to `@t2000/{sdk,engine}@2.14.x`~~ — **✅ SHIPPED S.260** (web-v2 on 2.14.1 via PR #105 / 2026-05-23 ~08:35 AEST). 2.14.0 broke SUI-source sponsored swaps (Enoki HTTP 400 `Cannot use GasCoin as a transaction argument`); patched in 2.14.1 with 3 SDK regression tests. Founder smoke verified all USDC-source flows + atomic compose (swap+save / repay+withdraw) work on 2.14.1. Final unblock target: `swap N SUI → USDC` (the regression target). Pending founder verification post-Vercel-deploy. | — | — |
| 4 | ~~**GW-REPRICE-1**~~ | ~~**Gateway sub-cent reprice** ~12 endpoints from $0.005 → $0.01~~ — **✅ SHIPPED S.259 (2026-05-23 ~07:30 AEST)** — actual scope was 45 routes (31× $0.005 chargeProxy + 4× $0.001 chargeProxy + 10× $0.005 chargeCustom) → all $0.01. 2 polling endpoints (`assemblyai/v1/result`, `replicate/v1/predictions/status`) preserved at $0.001 by design. Vercel auto-deploys from commit `1d00c3da`. | — | — |
