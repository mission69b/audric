...
> Last refreshed 2026-05-23 ~17:30 AEST post-S.277 (the "Earns Its Keep" audit + cuts — engine 2.18.0 pending release). Earlier today: S.273 (payment-link URL prefix fix shipped, audric `88ae4dc`); S.269 (template-divergence + V07E_INVOICE_DEPRECATION); S.272 deferred (BlockVision/DeFi cron rate-limits); S.267 (engine 2.15.0). Originally rewritten 2026-05-22 ~17:30 AEST post-S.255. Previous Phase-6.5-era detail is preserved in git history (`git log -- HANDOFF_NEXT_AGENT.md`); the document is now a tight current-state-of-the-world view, not a session-by-session log. Session-by-session detail lives in `t2000/audric-build-tracker.md`.

---

## 🎯 Where we are right now (1-paragraph)

**`audric.ai` serves `apps/web-v2` end-to-end. Engine 2.18.0 release pending after S.277 cuts; pre-release, web-v2 is on `@t2000/{sdk,engine}@2.17.0`. S.277 shipped 2026-05-23 ~17:30 AEST — the "Earns Its Keep" audit cut 5 engine tools** (Volo trio + `web_search` + `protocol_deep_dive`) **+ 2 dead post-cut guards** (`costWarning`, `artifactPreview`) **+ 1 dead flag** (`costAware`)**. `explain_tx` kept but description tightened (arbitrary external digests only) and primary system-prompt steer dropped. Tool count 31 → 26 (18 read + 8 write). SDK + CLI + MCP retain Volo for non-Audric consumers — only Audric's surface shrinks. `BRAVE_API_KEY` is dropped from `lib/env.ts`; operators can delete it from Vercel. Founder ops queue grows by zero (clean cut). Engine release workflow next (`gh workflow run release.yml --field bump=minor` from t2000); then web-v2 bumps to engine 2.18.0 and we externalize the audit narrative.**
...
| 0.7 | **S.272** | **BlockVision + DeFi cron rate-limits + abort hangs (DEFERRED)** | ~1d (3 levers) | Founder triage; SPEC needed | Logs from 02:30 UTC `financial-context-snapshot` cron showed circuit breaker OPEN events + 9-protocol DeFi adapter `HTTP 429` storms + AbortError flurries + final Vercel 300s task timeout. ~6 user snapshots skipped per cron run. Existing sticky-positive cache (15s fresh / 30min sticky) + circuit breaker (10×429/5s, 30s cooldown) prevent the bad data poisoning user-facing chat. Cron-skip messages observable via `walletSource=sui-rpc-degraded` + `defiSource=partial\|blockvision` flags. **3-lever fix:** (1) cron user-batching with intra-batch delay (10/5s instead of fanout); (2) per-adapter `Retry-After` header propagation to cache TTL; (3) AbortController coordination — when BV circuit opens, abort already-inflight DeFi calls for the same address. NOT folded into S.269 (different domain — vendor reliability not template divergence). Standalone SPEC needed; ship after S.274 closes. |
...
1. **Read this file** + the most recent ~3 entries in `t2000/audric-build-tracker.md` (S.272 / S.273 / S.274). Together they're the current state.
2. **The next agent-ownable work, in order:**
   ...
   - **(b) S.272 — BlockVision/DeFi cron rate-limits.** ~1d. 3-lever fix: cron user-batching, Retry-After propagation, AbortController coordination on circuit-open. Standalone SPEC needed.
   - **(c) PIPELINE-AUDIT-PHASE-2** — S1+S2+S3+S5 from `t2000/spec/active/AUDIT_ON_CHAIN_PIPELINE_2026-05-23.md`. ~4-5d. Coordinate with S.272 since both touch `blockvision-prices.ts`.
