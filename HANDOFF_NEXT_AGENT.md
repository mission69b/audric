...
| Engine | `@t2000/engine@2.17.0` (web-v2 ✅ on 2.17.0 post-S.269), 31 tools (21 read + 10 write — S.269 deleted 3 invoice tools per V07E_INVOICE_DEPRECATION), 9 canvas templates. S.267 added `[receive] tool=… status=… url=… detail=…` failure-path observability. S.269 introduced typed `ToolContextEnv` + deleted dead `saveContactTool` / `T2000_AUDRIC_API` legacy alias. |
...
|---|---|---|---|---|---|
| 0 | **S.269-SMOKE** | **Verify S.269 + V07E_INVOICE_DEPRECATION on prod** | ~5 min founder | Vercel deploy from audric `<post-S.269 commit>` + engine 2.17.0 npm propagation (~5 min) | (a) `create a payment link for $1 USDC` → `<PaymentLinkCard>` with QR + URL. (b) `list all payment links` → renders user's list (only works if `T2000_INTERNAL_KEY` is set in Vercel — boot will fail otherwise per env hardening). (c) `create an invoice for $200 for design work` → engine routes to `create_payment_link` with description telegraphing invoicing context; `<PaymentLinkCard>` renders with the invoice context encoded in label/memo. (d) Toggle a chat private/public → no `Unauthorized` error. (e) Delete all chats from `/settings/passport` → sidebar refreshes immediately. (f) Optional: `SELECT COUNT(*) FROM "Payment" WHERE type='invoice'` should return 0 (verified post-deploy). |
| 0.5 | **S.272** | **BlockVision + DeFi cron rate-limits + abort hangs (DEFERRED)** | ~1d (3 levers) | Founder triage; SPEC needed | Logs from 02:30 UTC `financial-context-snapshot` cron showed circuit breaker OPEN events + 9-protocol DeFi adapter `HTTP 429` storms + AbortError flurries + final Vercel 300s task timeout. ~6 user snapshots skipped per cron run. Existing sticky-positive cache (15s fresh / 30min sticky) + circuit breaker (10×429/5s, 30s cooldown) prevent the bad data poisoning user-facing chat. Cron-skip messages observable via `walletSource=sui-rpc-degraded` + `defiSource=partial\|blockvision` flags. **3-lever fix:** (1) cron user-batching with intra-batch delay (10/5s instead of fanout); (2) per-adapter `Retry-After` header propagation to cache TTL; (3) AbortController coordination — when BV circuit opens, abort already-inflight DeFi calls for the same address. NOT folded into S.269 (different domain — vendor reliability not template divergence). Standalone SPEC needed; ship after v0.7c soak gate clears. |
| 1 | ~~**S.267-SMOKE**~~ | ~~Verify S.267 payment-link + invoice fix on prod~~ — superseded by S.269-SMOKE (rank 0) which exercises the same flows post-deprecation | — | — |
