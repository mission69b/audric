...
|---|---|---|---|---|---|
| 1 | **S.263-SMOKE** | **Re-smoke compound write with @audric recipient** — verify bundle resolves | ~5 min founder | None | Post-S.263 Vercel deploy. Run `swap 5 USDC to SUI then send 1 SUI to funkiii@audric` on `audric.ai`. Expect: ONE permission card, ONE PTB on Suiscan, no error retries. Bonus smokes: `send 1 USDC to alice.sui` (single, .sui form) + `send 1 USDC to alice@audric` (single, @audric form) — regression check on the single-send path. Negative path: `send 1 USDC to bogus@audric` → expect 400 with helpful "isn't a registered SuiNS / Audric handle" message. |
| 2 | ~~**S.262-SMOKE**~~ | ~~Re-smoke compound writes on prod~~ — **✅ PARTIAL** (founder smoke 2026-05-23 ~09:30 AEST). `swap 1 SUI for USDC then save into NAVI` ✅ ONE PTB, ONE tap. `swap 5 USDC + send` failed at composeTx with non-hex recipient — patched in S.263 (PR #108). The atomic compound writes prompt fix itself is solid. | — | — |
| 3 | ~~**S.261-SCHEMA-TRIM**~~ | ~~Drop 4 dead `User` columns + tos-accept route~~ — **✅ SHIPPED S.261** (PR #106 / 2026-05-23 ~09:00 AEST). Schema + indexes physically dropped from prod Neon via Vercel-deploy migration. `pnpm prisma migrate diff` confirms migration matches schema. Zero behavior change (every consumer was already JWT-derived or DOA). | — | — |
| 4 | ~~**AUDRIC-BUMP-1**~~ | ~~Bump web-v2 to `@t2000/{sdk,engine}@2.14.x`~~ — **✅ SHIPPED S.260** (web-v2 on 2.14.1 via PR #105 / 2026-05-23 ~08:35 AEST). 2.14.0 broke SUI-source sponsored swaps (Enoki HTTP 400 `Cannot use GasCoin as a transaction argument`); patched in 2.14.1 with 3 SDK regression tests. | — | — |
| 5 | ~~**GW-REPRICE-1**~~ | ~~**Gateway sub-cent reprice** ~12 endpoints from $0.005 → $0.01~~ — **✅ SHIPPED S.259 (2026-05-23 ~07:30 AEST)** — actual scope was 45 routes (31× $0.005 chargeProxy + 4× $0.001 chargeProxy + 10× $0.005 chargeCustom) → all $0.01. 2 polling endpoints (`assemblyai/v1/result`, `replicate/v1/predictions/status`) preserved at $0.001 by design. | — | — |
| 6 | **CLI-CONTACTS-CLEANUP** | **Add SuiNS to `T2000.send()` + remove `contacts.json` legacy** | ~3-5h | Folds into SDK-ARCH-REVIEW | New backlog item from S.263 architectural choice. CLI's `T2000.send()` (`packages/sdk/src/t2000.ts:568`) currently resolves recipients via local `contacts.json` (`packages/sdk/src/contacts.ts:70`) — two name-resolution systems in the SDK (CLI contacts AND nothing for SuiNS). The audric-side fix (S.263) deliberately did NOT touch this because it's a separate concern: (a) `contacts.json` is a CLI-only artifact (audric web has no filesystem), (b) SDK's `composeTx.send_transfer` strict-hex contract is intentional. Scope: add SuiNS resolution to `T2000.send()` (accepts hex / `.sui` / saved-contact, in that priority order) + ship a deprecation path for `contacts.json` (warn on read, sunset in next major). Also worth considering: should `composeTx.send_transfer` accept SuiNS? Probably not — keep it strict-hex, let callers resolve. Document the contract clearly in SDK README. |
| 7 | **SDK-ARCH-REVIEW** | **SDK / CLI architecture review** (founder-flagged) | TBD scoping | Founder-owned scoping | Founder said: *"i feel like our sdk or cli is heavily and complicated and might need a separate review of its design and architecture."* The S.258 work added two new layers (build-time intent resolution via `coinWithBalance`; dual-client gRPC-build-then-JSON-RPC-execute for gasless detection) that grew complexity without simplifying anything. CLI-CONTACTS-CLEANUP (#6) folds in here as a concrete deliverable. Worth a half-day "what could we delete?" pass. |
| 8 | **MPP-1 follow-ups** | Free-tier endpoint demo only | ~5min cut | None | Most MPP-1 plumbing shipped in S.257 + S.258 + S.259. Open: gateway endpoint at `price: '0.000'` to demonstrate `@suimpp/mpp@0.7.0`'s free-tier protocol surface (PR #4 from manolisliolios). 5-min PR + Vercel deploy. SPEC 26 simplifications mooted by S.258's wholesale revert. |
| 9 | ~~**MCP-1**~~ | ~~Reinstate `pay_api` in `@t2000/mcp`~~ — **✅ ALREADY SHIPPED** (closed S.256 / 2026-05-22) | — | — | **The S.255 §5 scoping was based on a faulty premise.** `t2000_pay` (write tool wrapping `agent.pay()`) + `t2000_services` (read tool hitting gateway directly) have always been alive in `@t2000/mcp`. **Action:** none. See S.256 for the audit. |
| 10 | **H3.2** | **Contacts Phase 2 — Prisma `UserPreferences.contacts` drop** | ~30 min | ~24h soak from S.243 ✅ complete | ✅ Already shipped as part of S.254 Prisma migration. **Verify:** `prisma db pull` against prod Neon should show the column gone. Likely just close as DONE. |
| 11 | **H3.4** | **Contacts Phase 4 — engine cleanup** | ~30 min | None | Delete `packages/engine/src/tools/contacts.ts` + `add-recipient.ts` from `@t2000/engine`, remove from `tool-flags` + `tool-policy` + `tools/index`, delete tests, bump minor + publish. Unused exports today — no functional impact, just dead code drag. |
| 12 | **H3.5** | **Contacts Phase 5 — send-history reverse-lookup audit** | ~0-2h | None | Audit web-v2 send-history rendering. IF it currently relies on contact-stored names → add live reverse-lookup at render (Audric directory + SuiNS, session-cached). IF send history already shows raw 0x or routes through `resolve_suins` live → $0 work. Audit-first ship. |
| 13 | **M1 / SPEC 31** | **CSP perimeter polish — SPEC 31** | ~6-9h + 24-48h Report-Only soak | Founder triage to lock SPEC scope | Per `spec/active/SPEC_31_SCOPING.md`. Highest-leverage agent-only ready-to-ship security slice. CSP nonces + missing directives + `securityheaders.com` A+ rating + companion `/api/mpp/payments` admin gate inline fix. Independent of v0.7e/v0.7f. |
| 14 | **D2** | **Phase 3.5 memory controls in `/settings/memory`** | TBD | MemWal SDK feature (`MemoryStore.forget()` etc. not yet exposed) | Per-record delete via `MemoryStore.forget()`, "explain why this fact was recalled" provenance, recall-frequency ranking. Phase 3 LITE shipped a read-only top-K disclosure surface; controls deferred. |
| 15 | **D6** | **`memwal-per-user-accounts`** | ~5-8d (depends on Q1+Q4 answers) | Founder triage; possibly Mysten coordination | Promote from founder-owned singleton (one `MEMWAL_PRIVATE_KEY`, per-user namespace strings) to per-user `MemWalAccount` factory. Reference impl: `MystenLabs/MemWal/apps/chatbot`. 4 open questions for SPEC scoping. |
| 16 | **M2** | **engine-fn-injection-refactor** | ~14-21h / 2-3 sessions | **REBASELINED:** wait until any remaining engine→audric self-fetches are localized. Audit `packages/engine/src/tools/*` for `process.env.AUDRIC_INTERNAL_API_URL` / `fetch(...AUDRIC...)` patterns FIRST — the scope may have shrunk to <5h | Founder triage | Original scope was 13 fetch sites across 7 tool files; post-S.253 most rewrites are dead. Re-audit before scoping. |
| 17 | **M3** | **engine-internal-key-final-delete** (`T2000_INTERNAL_KEY` env var retirement) | ~30 min | Blocked on M2 | Final cleanup. Drop `validateInternalKey` + `/api/internal/payments` route + env var schema entries. |
| 18 | **B1** | **Marketing landing — shadcn redesign** | ~6-10h | Post-DNS-flip (✅ now done — gated open) | The 15 components ported at S.253 (`apps/web-v2/components/landing/`) are excluded from Biome lint. L-4 lock still applies (copy is legal-vetted, only UI changes). Drop `!components/landing` from `biome.jsonc` once done. |
| 19 | **D1** | **V07E_INVOICE_DEPRECATION** | ~4-5h / 5 phases | Founder priority restoration | Mini-SPEC at `spec/active/V07E_INVOICE_DEPRECATION.md` — drafted S.239, OUTBOUND-interface fix shipped, deeper deprecation waits. Drops invoice as a distinct product (~95% overlap with payment links). |
| 20 | **D3** | **V07F_FORWARD_MAP (Agentic Commerce Phase 1)** | ~10-14 calendar days | D-1 lock + post-v0.7e Phase 4 close | 4 streams in `spec/active/V07F_FORWARD_MAP.md`. Stream A (single-vendor `pay_api` revival in web-v2 — Audric-chat-internal MPP buying) is the remaining unsolved piece. |
| 21 | **D4** | **v0.7g Agentic Commerce Phase 2-4** (multi-vendor + delivery + creator + escrow) | ~13-17d | Post-v0.7f | Audric Store launch dependency. |
