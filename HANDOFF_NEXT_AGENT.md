...
| `apps-web-dead-rewritten-routes-cleanup` | ✅ MOSTLY CLOSED 2026-05-21 / S.229 (audric `0f53ae3`) — 3 of 4 dead-rewritten routes deleted: `/api/internal/payments` (-267 LoC), `/api/analytics/portfolio-history` (-165 LoC), `/api/analytics/activity-summary` (-50 LoC). Plus the `internal/` parent directory (no other internal routes left in apps/web). Net: -480 LoC, 0 functional impact (rewrites continue to serve from web-v2; post-deploy smoke confirms 3-hop x-vercel-id chain). Combined with S.228: -786 LoC across 5 routes. The 4th route (`/api/portfolio`) deferred to the row below due to a SPEC 30 regression test import. | Closed |
| `apps-web-portfolio-deletion-with-test-migration` | **NEW** OPEN (P2, ~45 min) — final dead-rewritten route deletion in apps/web. `/api/portfolio` is dead-rewritten by `apps/web/next.config.ts:140` → web-v2, but has 1 test import: `apps/web/__tests__/spec30-cache-header-regression.test.ts:84` → `await import('../app/api/portfolio/route')` (guards against re-introducing `Cache-Control: public` on the route — the 2026-05-14 production CDN cache-poisoning vuln). web-v2 has **no equivalent regression test today** even though web-v2's route already uses the correct `private` header. Slice: (1) port the regression test from apps/web/__tests__/ → apps/web-v2/__tests__/ adjusting import + mock paths, (2) verify it passes against web-v2's route, (3) delete the apps/web test, (4) delete `apps/web/app/api/portfolio/route.ts` + empty directory, (5) typecheck/lint/test/commit/push/smoke. After this slice ships, apps/web's entire dead-rewritten-route surface (6 routes across S.228+S.229+this slice) is gone. | Pre-v0.7e |
