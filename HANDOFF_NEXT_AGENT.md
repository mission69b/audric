...
**v0.7e Tier C Migration — Phase 1A SHIPPED (S.238, 2026-05-21 ~20:30 AEST)** + **Invoice deprecation kickoff (S.239, 2026-05-21 ~21:00 AEST)** + **Phase 5 RESTORED (S.245, 2026-05-22 ~07:50 AEST — D-2 reframe to full delete)** + **H2 Persistent Chats SHIPPED end-to-end (S.247, 2026-05-22 ~13:00 AEST — Prisma schema + 6 phases + ~30 template-debris files deleted; net ~-6,000 LoC)** + **H2 post-ship audit + 2 P0 + 11 P1 fixes (S.248, 2026-05-22 ~16:30 AEST — saveMessages upsert, client-rendered /chat/[id], orphan-row purge, vote UI restored; LOCK-4 deferred to v0.7f with paper trail)** + **🎯 PHASE 5 CLOSED — apps/web DELETED (S.253, 2026-05-22 ~22:00 AEST — DNS flipped to web-v2 ~21:30 AEST; prisma schema + migrations + generated client `git mv`'d into web-v2; `lib/prisma.ts` rewired to local imports; chat-route `Prisma` namespace import collapsed; `prisma` devDep + `postinstall: prisma generate` + `migrate` scripts added; 2 regression-swap CI workflows deleted; 678 files removed under `git rm -rf apps/web`; typecheck+lint+build all green)**
[`t2000/spec/active/BENEFITS_SPEC_v07e.md`](../t2000/spec/active/BENEFITS_SPEC_v07e.md). v1.0 LOCKED-PENDING-FOUNDER per S.237; Phase 1A executed under "agent-assumed defaults" path immediately after Phase 7 closed (G1 → CLOSED). **4,233 net LoC deleted from apps/web across 22 files in 2 batches.** Slices 1A.2 (voice) + 1A.3 (build-id) DEFERRED to Phase 1B/2 due to chat-shell entanglement; will delete naturally when chat-shell migrates. **Major scope change UPDATED per S.245 D-2 reframe:** Phase 5 (final apps/web archive) **RESTORED to v0.7e scope** (was deferred S.237 → S.244, reframed S.245 to full delete now). The legacy MPP `pay_api` capability returns as a clean-slate Commerce primitive in the upcoming Audric Store SPEC — NOT a port of the legacy 3-leg apps/web flow. v0.7e now ships Phases 1A → 2 → 3 → 4 → 5; apps/web dies en-bloc at Phase 5.
...
| D-2 | `services/*` migrate or delete? | ~~DEFER to v0.7f~~ → **REFRAMED S.245 → DELETE ENTIRELY NOW.** Engine `pay_api` + `mpp_services` tools deleted (v2.12.0); Audric Store SPEC owns the clean-slate Commerce redesign. v0.7e Phase 5 RESTORED (apps/web archives en-bloc). |
...
| 🟠 RE-BASELINED + SUBSTANTIALLY SHIPPED S.253 (5 commits 2026-05-22 ~19:00–21:00 AEST) | H1 | **v0.7e Phase 2 → STRATEGIC RE-FRAMING per S.253 audit (2026-05-22 ~17:30–18:30 AEST). Then 5 commits SHIPPED tonight (~19:00–21:00 AEST) closing the cron + legal + litepaper + marketing landing gaps. Only chat-shell rewrites + DNS flip remain.**

**SHIPPED 2026-05-22 ~21:00 AEST — 5 commits, ~5,000 LoC net additions to web-v2:**
1. `e7e8391` — UFC cron port (financial-context-snapshot route + job helper + CRON_SECRET env wiring). Dual-write window intentional — both apps' crons race idempotent upserts.
2. `97e495a` — remaining 4 crons port (portfolio-snapshot + turn-metrics-pending-sweep + turn-metrics-cleanup + conversation-log-retention). All 5 production crons now live in web-v2/vercel.json.
3. `49f6d07` — (legal) + litepaper static-route port. /disclaimer, /privacy, /security, /terms, /litepaper all build clean as static pre-rendered routes. 3 image assets copied to web-v2/public/litepaper/.
...
3. ~~**Post-swap (within 24h soak):** `git rm -rf apps/web && rm apps/web` — both the directory and its Vercel project.~~ ✅ **SHIPPED 2026-05-22 ~22:00 AEST (S.253).** Founder confirmed DNS swap landed clean (`audric.ai` serving web-v2 end-to-end); proceeded with `git rm` immediately. **Pre-delete migrations:** `apps/web/prisma/schema.prisma` + 37 migrations + `apps/web/lib/generated/prisma/` all `git mv`'d into `apps/web-v2/prisma/` + `apps/web-v2/lib/generated/prisma/` (preserves git history). `apps/web-v2/lib/prisma.ts` rewired from `../../web/lib/generated/prisma/client` → `./generated/prisma/client` (both `PrismaClient` + `Prisma` namespace re-export); load-bearing doc-block rewritten to reflect web-v2 as canonical schema owner. `apps/web-v2/app/api/chat/route.ts` collapsed its standalone `import { Prisma } from "../../../../web/lib/generated/prisma/client"` into the existing `prisma` import line. `apps/web-v2/package.json` gained `prisma@^7.5.0` devDep + `postinstall: prisma generate` + `migrate: prisma migrate deploy` scripts. `apps/web-v2/biome.jsonc` excludes `!lib/generated` (auto-gen, never hand-edit). **CI:** `.github/workflows/regression-swaps.yml` + `regression-swaps-execute.yml` deleted (both path-triggered on `apps/web/**` only — dead workflows post-delete; per Tier B's note re: `REGRESSION_TEST_WALLET_PRIVKEY` secret, re-port to web-v2 is optional follow-up). **`git rm -rf apps/web` removed 678 files.** Build/typecheck/lint all green post-deletion (`pnpm --filter @audric/web-v2 build` succeeds; all 6 ported API routes register correctly; root `/` static-prerendered serves marketing landing for unauthenticated users). **Vercel `audric-web` project retained for 7d rollback insurance** — domainless, no traffic, no maintenance burden. Delete from dashboard after the soak. Engine tools `pay_api` + `mpp_services` already deleted in v2.12.0 per S.245 Audric Store reframe.
4. **Backlog (DEFERRED, post-DNS-flip)** — marketing landing shadcn redesign per backlog row B1.

**Revised end-to-end (post-S.253): v0.7e Phase 5 CLOSED. apps/web is GONE. The only post-deletion cleanup left is (a) delete `audric-web` Vercel project in ~7d (pure dashboard click; no code change), (b) optionally re-port Tier B regression-swap harness to web-v2 if you want nightly on-chain swap monitoring back. Tier A swap regression (Cetus quote scenarios, no on-chain mutation) can be ported in ~1-2h if desired — purely a dev convenience, no production gap. Down from ~3-5d at session start; down from 25-36d at original framing. From here forward, web-v2 IS audric.**

----

**Original S.253 reframe context (preserved for traceability):** Two layers of correction this session: (1) First audit (against SPEC) claimed 3/5 Phase 6.5 P0s shipped + 2 partial. (2) Reading the TRACKER (which I should have done first) revealed Two layers of correction this session: (1) First audit (against SPEC) claimed 3/5 Phase 6.5 P0s shipped + 2 partial. (2) Reading the TRACKER (which I should have done first) revealed **ALL of Phase 6.5 Groups A+B+C SHIPPED 2026-05-20 in S.199/S.200/S.201** with intentional design deviations (rate-limit stays in-memory by design; ChatGate inline via AuthenticatedChat wrapper). The SPEC's "NOT STARTED" header was never patched. See `spec/active/SPEC_V07C_PHASE_6_5_CHAT_PARITY.md` §0.1 for the corrected truth table. **Compounding correction:** v0.7c is essentially closed (S.215: *"7d soak collapsed into Group E smoke + production verification; web-v2 production has been native-HITL-only since the v0.7c Phase 6.5 chat-flip"*). v0.7d MemWal landed in Phase 6 Block A (S.221). v0.7e Phase 1A shipped (S.238). Outstanding work was framed as: smoke check + chat-shell rewrites + port marketing/legal/litepaper/crons + DNS flip + apps/web archive. **The smoke ambiguity was resolved this session via WebFetch (see Live production findings above) — chat-flip rewrites NOT shipped, founder ops step still pending.** Cron + (legal) + litepaper gaps closed tonight. Marketing landing port + chat-shell rewrites + DNS flip + apps/web archive remain. | **~1-2d remaining + founder ops + soak** (down from ~3-5d at session start; down from 25-36d at original framing) | Marketing landing port decision (full port vs minimal placeholder); chat-shell rewrites in apps/web/next.config.ts (founder-gated for production routing impact) |
| ✅ DONE | H2 | **SHIPPED 2026-05-22 — v0.7e Persistent Chats end-to-end** (S.247 + S.248 post-ship audit). All 6 phases across ~7h: Phase 0 audit, Phase 1 schema + persistence + chat-route wiring + title generation, Phase 2 (chat) route group fold + ~30 template debris files deleted (drizzle + artifacts + lib/db + lib/editor), Phase 3 click-to-resume `/chat/[id]/page.tsx`, Phase 4 visibility toggle + `/share/[id]/page.tsx` + SharedChatViewer + copy-link, Phase 5 polish (sonner Toaster mount + stable client-side id + `window.history.replaceState` URL promote + Next 16 `<Suspense>` pattern for cacheComponents compat + branded 404). Net ~-6,000 LoC. Prisma migration deployed to prod Neon. **S.248 post-ship audit (~2.5h) fixed 2 P0s (saveMessages upsert for HITL continuations + client-rendered /chat/[id] for RSC JWT issue) + 11 P1s (orphan rows, updatedAt ordering, visibility race, title PII redaction, vote messageId guard, sidebar copy, null-title fallback, vote UI restored).** LOCK-4 (live-stream resume via StreamCheckpointStore) DEFERRED to v0.7f with paper trail. Typecheck + lint + build all green. **G1-G5 founder smoke deferred — needs preview/prod deploy.** | DONE | — |
...
| 🟤 DEF | B1 | **Marketing landing — shadcn redesign (post-DNS-flip)** — the 15 components ported at `06682b8` (S.253) live in `apps/web-v2/components/landing/` and are excluded from Biome lint per the `components/audric/cards` precedent. The verbatim port carries forward 3 patterns that should be intentionally rewritten when the marketing site gets its v2 design pass: (1) PascalCase filenames → kebab-case (matches rest of web-v2 convention); (2) `dangerouslySetInnerHTML` in `HeroChatWidget` for inline `<b>`/`<br>` formatting → JSX with proper React fragments; (3) array-index keys on the static demo-flow array → stable string keys (e.g. flow.key + msg index slug). When ready: rebuild each section with shadcn primitives (Card, Button, etc.) preserving the marketing copy verbatim (L-4 lock still applies — copy is legal-vetted, only the UI changes). Drop the `!components/landing` line from `biome.jsonc` once the redesign re-enters convention. Could also fold in: replace `useStats` SWR with @tanstack/react-query for parity with rest of web-v2; add proper Image components for any logo/screenshots that get added in the redesign. **Effort: ~6-10h (design + 15 component rewrites + visual QA against current site); recommended timing: post-DNS-flip when there's no migration pressure.** | ~6-10h | Post-DNS-flip; no other blockers |
...
| 🟤 DEF | D6 | **`memwal-per-user-accounts`** — promote `apps/web-v2/lib/memwal.ts` from founder-owned singleton (one `MEMWAL_PRIVATE_KEY` + `MEMWAL_ACCOUNT_ID` in Vercel env, per-user namespace strings `audric:user:<addr>` for logical scoping) to a per-user MemWalAccount factory (each user gets their own `MemWalAccount` on Sui, auto-provisioned at zkLogin sign-up via `createAccount()` + `addDelegateKey()` from `@mysten-incubation/memwal`; namespace collapses to `"default"` per-account). **Why this matters:** today the founder's MemWal key technically holds the keys to every user's encrypted memory — relayer enforces namespace scoping but a stolen `MEMWAL_PRIVATE_KEY` could enumerate every user's recall set. Per-user accounts give true crypto-level isolation (separate Sui objects, separate Walrus blobs encrypted to each user's keys). **Status:** today the future-promotion plan exists ONLY as inline doc-blocks in `lib/memwal.ts` L21-38 + `memwal-memory-store.ts` L58-66 — no spec, no backlog row before this one. **Reference impl:** `MystenLabs/MemWal/apps/chatbot` shows the per-user provisioning pattern audric should mirror. **Open questions for SPEC scoping:** (1) where does the user's delegate private key live — derived from zkLogin like Enoki sponsorship, sponsored by audric, or user-held in localStorage? (2) provisioning latency budget — auto-provision blocking on first sign-up vs lazy on first chat turn? (3) migration path for existing namespace-scoped recall sets — re-ingest from chat history, or accept memory loss at cutover? (4) Mysten coordination — does the MemWal SDK contract for `createAccount()` need extension? **Risk surface lives since 2026-05-21 (S.215 MemWal production-wire).** SPEC 30 v1.0 LOCKED 2026-05-14 — 7 days BEFORE MemWal landed — so the cross-repo security review never scored this. | TBD (likely ~5-8d once scoped; depends on Q1 + Q4 answers) | Founder triage; possibly Mysten coordination on SDK contract |
...
3. **Then by priority**: ~~H3 contacts (per locked path) → H2 persistent chats Phase 1 schema~~ ✅ BOTH SHIPPED → **H1 v0.7e Phase 2** (chat-shell cutover, the last big v0.7e structural blocker) → G1-G5 founder smoke against next preview deploy
