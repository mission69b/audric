...
| 🟢 READY | H1 | **v0.7e Phase 2** — chat-shell cutover + 1A.2/1A.3 absorption + fn-injection refactor. **ALL LOCKS STAMPED + PHASE 2.0 SPIKE SHIPPED 2026-05-22 ~15:30 AEST (S.252).** Per `spec/active/V07E_PHASE_2_PRE_EXECUTION_AUDIT.md`: Q1 → **L-8A (configure vitest in web-v2)**; Q2 → **KILL ALL 6 `/api/engine/*` routes (no feature loss — all have structural v2 replacements)**; Q3 → **DELETE legacy `/api/history` (TX history; dies naturally with Phase 2.5 fn-injection + Phase 6)**; D-3 voice → **RATIFIED DELETE**; D-6 build-id → **RATIFIED DELETE**; scope-split → **SINGLE BLOCK (founder pick; risk noted)**. Post-lock effort revised down to **6-8d realistic** (Q2/Q3 turned out cheaper than worst-case). **Phase 2.0 vitest spike landed:** `apps/web-v2/{vitest.config.ts,vitest.setup.ts,lib/utils.test.ts}` shipped; `pnpm --filter @audric/web-v2 test` → 7 tests pass in 1.04s. Vitest + jsdom installed in web-v2 devDeps. `lib/ai/models.test.ts` (Vercel template debris — mock fixtures with `.test.` naming but zero test cases / zero importers) excluded from collection; deletion is separate housekeeping. **Next session opens directly to Phase 2.1 (`git mv apps/web/lib/engine/* web-v2/lib/engine/*`).** | ~6-8d single block | None — founder locks all stamped, vitest infra proven, ready to code |
