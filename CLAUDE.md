...
| 🧠 **Audric Intelligence** | Brain (the moat) — 5 systems orchestrate every money decision. Engineering-facing brand; users experience it as "Audric just understood me." | `@t2000/engine` (37 tools, reasoning, guards) + `@t2000/mcp` (14 skills as MCP prompts) + audric-side `record_advice` + silent context (`engine-context.ts`, `<financial_context>` daily snapshot from `UserFinancialContext`) | Live |
...
User types message
  ...
  → engine-factory.ts: AISDKEngine → Claude with 37 tools (25 read + 12 write, BlockVision-backed pricing via `token_prices` / `balance_check` / `portfolio_analysis`)
